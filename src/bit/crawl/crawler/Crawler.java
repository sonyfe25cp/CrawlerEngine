...
public class Crawler implements Runnable, ICrawlerForWorker {
	...
	// private ThreadSafeClientConnManager connManager = new
	...
	// private String bloomPath =
	// "/Users/omar/software/crawlerengine/jobs/zhaopin-crawler.bf";//单个bloomFilter文件地址
	...
	public List<String> resetInitialUrls() {
		...
		if ((begin != 0) && (end != 0) && (begin < end)) {// 通过begin/end配置抓取多日新闻
						...
						for (FilterRule fr : original) {
							...
							if (pstring.contains("#aimStoreFormat#")) {
								pstring = pstring.replaceFirst(
										"#aimStoreFormat#",
										timeFormat.getStoreTimeFormat());
								System.out
										.println("#######replaced pattern content is "
												+ pstring);
								...
								pstring = pstring.replaceFirst(
										"#aimFollowFormat#",
										timeFormat.getFollowTimeFormat());
								// System.out.println("########replaced pattern content is "
								// + pstring);
								fre.setPattern(Pattern.compile(pstring));
								frs.add(fre);
			...
			setFilterRules(frs);
		} else { // 新加///////////////////convert InitialUrls into standard
					// format
			for (String link : getInitialUrls()) {
			...
			for (FilterRule fr : getFilterRules()) {
				for (Pattern p : fr.getPatterns()) {
					...
					pstring = p.toString();
					if (pstring.contains("#aimStoreFormat#")) {
						pstring = pstring.replaceFirst("#aimStoreFormat#",
								timeFormat.getStoreTimeFormat());
						fr.setPattern(Pattern.compile(pstring));
					} else if (pstring.contains("#aimFollowFormat#")) {
						pstring = pstring.replaceFirst("#aimFollowFormat#",
								timeFormat.getFollowTimeFormat());
						fr.setPattern(Pattern.compile(pstring));
					}
				}
	...
	public void run() {
		try {
			...
			logger.info("Crawling somthing happened.");
		} finally {
			if (executor != null) {
				executor.shutdown();
				try {
	...
	public void reportPageFetched(PageInfo pageInfo) {
		...
		if (getFilterAction(url) == CrawlAction.STORE
				|| getFilterAction(url) == CrawlAction.FOLLOW_STORE) {
