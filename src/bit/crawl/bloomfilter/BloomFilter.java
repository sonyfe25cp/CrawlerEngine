...
public class BloomFilter<E> implements Serializable {
    ...
    static { // The digest method is reused between instances                        加密
        MessageDigest tmp;
        try {
...
System.out.println("c:the number of bits used per element = " + Math.ceil(-(Math.log(falsePositiveProbability) / Math.log(2))) / Math.log(2));
System.out.println("k:the number of hash functions used = " + (int)Math.ceil(-(Math.log(falsePositiveProbability) / Math.log(2))));
    }
